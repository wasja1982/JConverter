JConverterGUI 0.5
=============================
Конвертор бекапа Ucoz в [AtomM CMS](http://atom-m.net) с графическим интерфейсом
...
2. Параметры командной строки:
  1. Общие параметры:
    ...
    * `-so "sitename"` - старое имя сайта в формате site.ucoz.ru или site.ru (без www);
    * `-sn "sitename"` - новое имя сайта (можно с www);
    * `-pof count` - число сообщений на форуме (используется при генерации ссылок на пост для старых версий).
